...
from utils import collide, text_bg

pygame.font.init()
WIN = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("Scene 3")
    ...
    def gameplay(self):
        ...
        while run:
            ...
            for i in range(panels):
                WIN.blit(bg, (0, i * bg_height + scroll - bg_height))
            ...
            if final:
                ...
                while pause:
                    ...
                    WIN.blit(win_bg, (0, 0))
                    WIN.blit(BUTTON_NEXT, (WIDTH / 2 - BUTTON_NEXT.get_width() / 2, 500))
                    if result:
                        VICTORY_SOUND.play()
                        label = lost_font.render(f"You Won!!!", True, (255, 255, 255))
                        WIN.blit(label, (WIDTH / 2 - label.get_width() / 2, 250))
                    ...
                    WIN.blit(label1, (WIDTH / 2 - label1.get_width() / 2, 320))

                    WIN.blit(label2, (WIDTH / 2 - label2.get_width() / 2, 390))

                    WIN.blit(label3, (WIDTH / 2 - label3.get_width() / 2, 460))
            ...
            if player.kill_count == enemies_count:
                if not showed_ship_a:
                    utils.add_to_db_sqlite(3, self.current_attempt, title1, desc1, 'ACHIEVEMENT_3_1', str(datetime.now())[:-7],
                                     'True')
                    showed_ship_a = True
                achievement_ships.draw_n_move(WIN, 5)
            ...
            if not space_station.alive:
                space_station.rect_red = pygame.draw.rect(WIN, (255,0,0), (0, 0, 0, 0))
                space_station.x = 0
